 ...
 * - Grid data preparation for the next step (edit.php)
 * - Loading saved state when returning from edit.php
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function saveGrid() {
        ...
        // Create a form and submit it to edit.php
        const form = document.createElement('form');
        form.method = 'post';
        form.action = 'edit.php';
        
        // For backward compatibility, keep the cells array as linear indices
        const backwardCompatGridData = {
            rows: gridData.rows,
            cols: gridData.cols,
        ...
        input.type = 'hidden';
        input.name = 'gridData';
        input.value = JSON.stringify(backwardCompatGridData);
    ...
    // Expose the loadReturnedGridData function to the global scope
    // This allows it to be called from inline scripts in index.php
    window.loadReturnedGridData = function(returnedData) {
        if (!returnedData) return;
        
        // Check if the data has the required properties
        if (returnedData.rows && returnedData.cols) {
            // Restore grid dimensions if they match current config
            if (returnedData.rows === config.rows && returnedData.cols === config.cols) {
                // Reset existing grid data
                clearGrid();
                
                // Restore coordinates (both formats for backward compatibility)
                if (returnedData.coordinates && Array.isArray(returnedData.coordinates)) {
                    gridData.cells = [...returnedData.coordinates];
                }
                
                if (returnedData.cells && Array.isArray(returnedData.cells)) {
                    gridData.cellIndices = [...returnedData.cells];
                }
                
                // Restore cell colors
                if (returnedData.cellColors && typeof returnedData.cellColors === 'object') {
                    gridData.cellColors = {...returnedData.cellColors};
                }
                
                // Restore Start/Finish positions
                if (returnedData.startPositions && Array.isArray(returnedData.startPositions)) {
                    gridData.startPositions = [...returnedData.startPositions];
                }
                
                if (returnedData.finishPositions && Array.isArray(returnedData.finishPositions)) {
                    gridData.finishPositions = [...returnedData.finishPositions];
                }
                
                // Update UI to match restored data
                updateGridUI();
                
                // Cache this grid as the custom grid
                cacheCustomGrid();
